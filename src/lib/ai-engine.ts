...
export class AIEngine {
    ...
    /**
     * Extract 12 features for Spot ML models:
     *   1. f_rsi: RSI / 100
     *   2. f_macd: MACD Histogram / Close
     *   3. f_ema20Dist: (Close - EMA20) / Close
     *   4. f_emaCross: (EMA20 - EMA50) / EMA50
     *   5. f_momentum: (Close - Close[i-3]) / Close[i-3]
     *   6. f_volatility: ATR / Close
     *   7. f_ema200Dist: (Close - EMA200) / Close
     *   8. f_mfi: MFI / 100
     *   9. f_obvChange: (OBV - OBV_EMA20) / OBV_EMA20
     *   10. f_bbSpread: Bollinger Band bandwidth
     *   11. f_adx: ADX / 100 (Trend strength)
     *   12. f_volChange: (Volume - Volume_EMA20) / Volume_EMA20 (Volume spike indicator)
     */
    extractFeatures(
        closes: number[],
        highs: number[],
        ...
        const volumeEma20 = this.calculateEMA(volumes, 20);
        ...
        for (let i = 200; i < length; i++) {
            ...
            const currentVolEma20 = volumeEma20[i];
            ...
            const f_adx = currentADX / 100;
            const f_volChange = (volumes[i] - currentVolEma20) / currentVolEma20;
    ...
    labelDatasetTripleBarrier(
        ...
        for (let i = 0; i < dataset.length; i++) {
            ...
            for (let j = 1; j <= forwardCandles; j++) {
                ...
                const lo = lows[idx + j];
                // Spot Long-Only: Touch lower barrier (SL) or double touch is HOLD (0)
                const tpHit = hi >= upper;
                const slHit = lo <= lower;
                if (tpHit && slHit) { label = 0; break; }
                if (tpHit) { label = 1; break; }
                if (slHit) { label = 0; break; }
            }

            out.push({ features: item.features, label, price });
        }
     ...
     */
    predictKNN(trainingData: LabeledDataPoint[], targetFeatures: number[]): PredictionResult {
    ...
    trainLogisticRegression(trainingData: LabeledDataPoint[], epochs = 500, lr = 0.03): LogisticRegressionModel | null {
        ...
        // Split: chronological 85% train / 15% validation for early stopping.
        const splitIdx = Math.floor(trainingData.length * 0.85);
        const trainData = trainingData.slice(0, splitIdx);
        const valData   = trainingData.slice(splitIdx);
        // Fall back to full dataset when the split is too small to be meaningful.
        const effectiveTrain = trainData.length >= 20 ? trainData : trainingData;
        const effectiveVal   = valData.length   >= 5  ? valData   : null;

        const numFeatures = effectiveTrain[0].features.length;
        const { means, stdDevs } = computeFeatureNormalization(effectiveTrain);

        const normalizedTrain = effectiveTrain.map(d => ({
            ...d,
            features: normalizeFeatureVector(d.features, means, stdDevs)
        }));
        const normalizedVal = effectiveVal ? effectiveVal.map(d => ({
            ...d,
            features: normalizeFeatureVector(d.features, means, stdDevs)
        })) : null;

        const sigmoid = (z: number) => 1 / (1 + Math.exp(-Math.max(-20, Math.min(20, z))));
        const lambda = 0.005; // reduced L2 — less over-regularization on small samples
        ...
        /**
         * Train one binary classifier (Long or Short) with early stopping.
         * Returns the weights + bias snapshot that achieved the lowest val loss.
         */
        function trainBinary(posLabel: number): { weights: number[]; bias: number } {
            ...
            for (let epoch = 0; epoch < epochs; epoch++) {
                for (let i = 0; i < normalizedTrain.length; i++) {
                    const features = normalizedTrain[i].features;
                    const label = normalizedTrain[i].label === posLabel ? 1 : 0;
                    const sampleW = normalizedTrain[i].weight ?? 1.0;
                ...
                if (normalizedVal) {
                    const valLoss = bce(normalizedVal, wt, b, posLabel);
        ...
        const longModel  = trainBinary(1);

        return {
            weightsLong:  longModel.weights,
            biasLong:     longModel.bias,
            weightsShort: [],
            biasShort:    0,
            featureMeans:   means,
    ...
    predictLogisticRegression(
        ...
        for (let i = 0; i < features.length; i++) {
            dotLong += features[i] * model.weightsLong[i];
        }

        const probLong = sigmoid(dotLong + model.biasLong);

        let signal = 0;
        let confidence = 50;

        if (probLong >= threshold) {
            signal = 1;
     ...
     */
    predictMomentumStrategy(closes: number[], highs: number[], lows: number[], volumes?: number[]): PredictionResult {
        const rsi = this.calculateRSI(closes, 14);
        ...
        if (isEmaBullishCross) scoreBuy += 2.0;
        if (isRsiOversoldCross) scoreBuy += 2.0;
        if (isMacdBullishCross) scoreBuy += 2.0;

        if (isEmaBearishCross) scoreSell += 2.0;
        if (isRsiOverboughtCross) scoreSell += 2.0;
        if (isMacdBearishCross) scoreSell += 2.0;
        ...
        if (currentRSI < 40 && currentRSI > prevRSI) {
            scoreBuy += 1.0; // RSI in oversold zone AND turning up
        } else if (currentRSI > 60 && currentRSI < prevRSI) {
            scoreSell += 1.0; // RSI in overbought zone AND turning down
        }
        ...
        if (ema200Dist > 0) { // Above EMA200: Bullish macro bias
            scoreSell *= (1 - trendStrength * 0.5); // Penalize Short up to 50%
            scoreBuy += 0.8 * trendStrength; // Bonus Buy proportional to distance
        } else { // Below EMA200: Bearish macro bias
            scoreBuy *= (1 - trendStrength * 0.5); // Penalize Buy up to 50%
            scoreSell += 0.8 * trendStrength; // Bonus Sell
        }
        ...
        if (scoreBuy >= 2.5 && scoreBuy > scoreSell) {
            ...
            signal = 0;
            confidence = Math.round(100 - scoreBuy * 12);
        }
